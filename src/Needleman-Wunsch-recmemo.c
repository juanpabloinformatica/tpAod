...
static void calcul_rec(long **tableau, char* A, char* B, size_t Ab, size_t Ae, size_t Bb, size_t Be, size_t lengthA, size_t lengthB) {
   size_t nb_A = Ae - Ab;
   size_t nb_B = Be - Bb;
   if (nb_A < 128 && nb_B < 128) {
      for (int iA = Ab; iA <= Ae; iA++) {
         for (int iB = Bb; iB <= Be; iB++) {
            long align = tableau[iA-1][iB-1] + kronecker(A, iA-1, B, iB-1);
            long delete = (isBase(A[iA-1]) ? 2 : 0) + tableau[iA-1][iB];
            long insert = (isBase(B[iB-1]) ? 2 : 0) + tableau[iA][iB-1];
            long min = align;
            if (delete < min) {
                min = delete;
            }
            ...
            if (!isBase(A[iA-1])) {
               tableau[iA][iB] = tableau[iA-1][iB];
            } else if (!isBase(B[iB-1])) {
               tableau[iA][iB] = tableau[iA][iB-1];
            } else {
               tableau[iA][iB] = min;
            }
         }
      ...
      size_t mid = nb_A / 2;
      calcul_rec(tableau, A, B, Ab, Ab + mid, Bb, Be, lengthA, lengthB);
      calcul_rec(tableau, A, B, Ab + mid, Ae, Bb, Be, lengthA, lengthB);
   } else {
      size_t mid = nb_B / 2;
      calcul_rec(tableau, A, B, Ab, Ae, Bb, Bb + mid, lengthA, lengthB);
      calcul_rec(tableau, A, B, Ab, Ae, Bb + mid, Be, lengthA, lengthB);      
   }
}
...
long EditDistance_NW_cache_oblivious(char* A, size_t lengthA, char* B, size_t lengthB)
{
   ...
   calcul_rec(tableau, A, B, 1, lengthA, 1, lengthB, lengthA, lengthB);
   return tableau[lengthA][lengthB];
}
